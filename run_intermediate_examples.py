...
2. 工程实践：模型训练、调优、压缩、分布式训练
3. LLM专项：架构原理、Prompt工程、微调、推理优化
...
if TORCH_AVAILABLE:
    try:
        from intermediate.models_torch import CIFAR10Net, QuantizedCIFAR10Net
...
# 尝试导入可选模块
try:
    from intermediate.kaggle_models import create_kaggle_model
    KAGGLE_MODELS_AVAILABLE = True
except ImportError:
    KAGGLE_MODELS_AVAILABLE = False

try:
    from intermediate.llm_architecture import LLaMAModel, create_llama_example
    from intermediate.llm_visualization import create_llama_visualization
    LLM_AVAILABLE = True
except ImportError:
    LLM_AVAILABLE = False
...
if TORCH_AVAILABLE:
    try:
        from intermediate.data import get_cifar10_loaders
        DATA_AVAILABLE = True
    except ImportError:
...
try:
    from intermediate.kaggle_data import get_kaggle_loaders
...
if TORCH_AVAILABLE:
    try:
        ...
        from intermediate.monitoring import PerformanceMonitor
...
if NUMPY_AVAILABLE:
    try:
        ...
        from intermediate.performance import benchmark_matmul
        SNN_AVAILABLE = True
    except ImportError:
        SNN_AVAILABLE = False
else:
...
# Dashboard相关导入
try:
    from intermediate.visualization import create_dashboard, load_data
...
# Pruning相关导入
try:
    from intermediate.pruning import ModelPruner, demonstrate_pruning
...
# Fine-tuning相关导入
try:
    from intermediate.finetuning import (
...
try:
    from intermediate.inference_optimization import (
...
def demonstrate_llm_architecture():
    ...
    print("\n3. 演示多头注意力...")
    try:
        from intermediate.llm_architecture import MultiHeadAttention
    ...
    print("\n4. 演示RoPE位置编码...")
    try:
        from intermediate.llm_architecture import RotaryPositionalEmbedding
    ...
    print("\n5. 演示残差连接和RMSNorm...")
    try:
        from intermediate.llm_architecture import RMSNorm, TransformerBlock
...
def demonstrate_fundamentals():
    ...
    print("\n🧮 第1部分：线性代数与自动微分")
    try:
        from intermediate.linear_algebra import demonstrate_comprehensive_linear_algebra
    ...
    print("\n🔄 第2部分：反向传播机制")
    try:
        from intermediate.backpropagation import demonstrate_comprehensive_backprop
    ...
    print("\n⚙️ 第3部分：优化算法对比")
    try:
        from intermediate.optimizer_comparison import demonstrate_comprehensive_optimizers
    ...
    try:
        from intermediate.cnn_transformer import demonstrate_comprehensive_cnn_transformer
